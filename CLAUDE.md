    ...
    "schools": [
      {
        ...
        "examDate": 20260225,
        "resultDate": 20260310,
        "enrollmentFeeDeadline": 20260317,
        "tuitionDeadline": 20260331,
...
詳細は `lean-backend/test_scenarios.json` を参照。2026年度入試データを使用。
...
- 東大: 未発表（発表: 3/10）
- 早稲田: 合格、未払い（期限: 3/6）
- **期待結果**: 何もしない（東大の結果待ち、早稲田期限まで余裕あり）
...
1. **日付はYYYYMMDD形式の整数で管理**
   - 例: 2026年2月25日 → `20260225`
   - 整数の大小比較がそのまま日付の前後関係となる
   - フロントエンドの`date-utils.ts`で`Date`型と相互変換
...
4. **実際の入試日程との整合性**
   - 早稲田の入学金期限(3/6)が東大発表(3/10)より前になるケースは実際に発生する
